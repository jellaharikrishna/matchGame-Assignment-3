...
class MatchGame extends Component {
  ...
  startTimer = () => {
    const {timer, isToggledImg} = this.state

    if (timer === 0) {
      clearInterval(this.timerID)
      this.setState({isToggledImg: !isToggledImg})
    } else {
  ...
  onToggleThumbnail = thumbnailUrl => {
    const {activeImgUrl, isToggledImg} = this.state
  ...
  onTogglePlayAgain = () => {
    const {isToggledImg} = this.state
    this.setState({
      activeTabId: tabsList[0].tabId,
      activeImgUrl: imagesList[0].imageUrl,
      score: initialScore,
      timer: initialTimer,
      isToggledImg: !isToggledImg,
    })
    this.timerID = setInterval(this.startTimer, 1000)
  }

  render() {
    const {activeTabId, activeImgUrl, isToggledImg, score, timer} = this.state
    ...
    return (
      <div className="app-container">
        <NavbarItem score={score} timer={timer} />
        {isToggledImg ? (
          <div className="gameover-container">
            <div className="score-card">
              ...
              <button
                ...
                onClick={this.onTogglePlayAgain}
              >
                <img
          ...
          <div className="content-container">
            <img className="active-image" src={activeImgUrl} alt="match" />
